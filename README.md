...
Given a neighbourhood (MSOA grain, 7,264 areas), it shows five 0–100 indicators —
affordability, safety, energy, flood resilience, convenience — **and the raw fact
behind every score**, with a weighted overall that re-ranks to the user's
priorities, per-area confidence, and source caveats. Not a price predictor and not
a listings site: an honest decision layer over fragmented official UK datasets.
Indicators only — never a "safe/unsafe" verdict.
...
| Surface | URL |
|---|---|
| 🌐 **Website** (Next.js / Vercel) | https://uk-housing-decision-support.vercel.app |
...
`decision.duckdb` extract; the API serves it; the website (and a planned mobile
app) are HTTP clients of the same typed contract.
...
```text
  ...
  (9 open-data sources)        (slim extract)            │  /v1 + OpenAPI
                                                         │
                                          ┌──────────────┴──────────────┐
                                   Website (Next.js, web/)      Mobile app (Expo)
                                      Vercel · live                 planned
```
...
| `web/` | **Next.js website** — search, compare, listing checker, and ~7k programmatic area/town/region/rent pages. See [`web/README.md`](web/README.md). |
| `app/` | *Legacy* Streamlit app (the original renter-facing UI; being retired now the website is live). |
...
- Show source freshness and coverage; make uncertainty visible.
...
- **Phase 2 — Website** ✅ **deployed** (Vercel): search, compare, listing checker,
  and ~7k programmatic area/town/region/rent pages with the internal-link mesh.
